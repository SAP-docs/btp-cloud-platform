...
Typically, a user is identified by email and origin \(your alias for the identity provider\). However, to most accurately identify a user, you need both user identifier and an identifier from an identity provider. The reason is that the system treats users with the same name but from different identity providers as separate users. For example, you can have a platform user in the default identity provider, SAP ID service, and another user in your corporate identity provider with the same e-mail address. This behavior applies to global accounts, directories, multi-environment subaccounts, and Cloud Foundry orgs and spaces. Users consist of a configurable identifier and of an identifier from an identity provider. Typically this is email and origin \(alias of the identity provider\). Two users with the same email but different identity providers are considered two different users because they are stored in different identity providers. Both can log on to the cockpit, but their user information is different because they are stored in different identity providers.
