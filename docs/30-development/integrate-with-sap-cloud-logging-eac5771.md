...
4.  Configure the [Istio](https://istio.io/latest/docs/reference/config/telemetry/) `Telemetry` resource \(see [Configure Istio Access Logs](configure-istio-access-logs-808c167.md)\) with the `kyma-logs` extension provider.
...
4.  Depending on the pipelines you are using, enable some or all of the following alerts:
    ...
    Monitors the `LogPipeline`. Triggers if log data stops flowing.
