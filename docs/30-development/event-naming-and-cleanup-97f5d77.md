...
-   Event: Typically, has two or three segments separated by a dot \(`.`\), like `order.created` or `Account.Root.Created`
...
To check the Subscription's cleaned event type, run:

```
kubectl get subscriptions {SUBSCRIPTION_NAME} -o=jsonpath="{.status.types}"
```
