...
4.  \(Optional\) Change the log level of a logging location.
    ...
    You can do this by using a file. You need to restart your application after that. Use cases:
    ...
    -   Spring Boot applications based on Java Main. Their default log configuration provides messages of levels ERROR, WARN, and INFO.

        -   To enable other log levels, for example DEBUG, create a file **`application.properties`** in directory `/src/main/resources/` and add the following line:
