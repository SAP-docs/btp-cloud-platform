...
The following code sample shows how you can generate a CDS dynamic external entity with the XCO library by running a `PUT` operation:
...
>  
> DATA(lo_form_specification) = lo_put_operation->for-ddls->add_object( 'ZMY_CDS_DYN_EXT_ENTITY'
...
Running this `PUT` operation will produce the CDS external entity `ZMY_CDS_DYN_EXT_ENTITY` with the following source:

> ### Sample Code:  
> ```
> @EndUserText.label: 'My external entity'
> define external entity ZMY_CDS_DYN_EXT_ENTITY external name "My remote object name"
> {
...
The XCO library also supports static external entity generation by specifying a logical external schema in the `set_provided_by_lgcl_ex_schema` method like this:
...
>   )->create_put_operation( ).
>  
> DATA(lo_form_specification) = lo_put_operation->for-ddls->add_object( 'ZMY_CDS_STATIC_EXT_ENTITY'
>   )->set_package( 'ZMY_PACKAGE'
>   )->create_form_specification( ).
>  
> DATA(lo_external_entity) = lo_form_specification->set_short_description( 'Generated CDS External Entity'
...
>  
> lo_external_entity->set_with_federated_data(
>   )->set_provided_by_lgcl_ex_schema( xco_cp_abap_repository=>object->desd->for( 'ZMY_LOGICAL_EXT_SCHEMA' ) ).
>  
> lo_external_entity->add_annotation( 'EndUserText.label' ##NO_TEXT
>   )->value->build( )->add_string( 'My external entity' ).
>  
...
Running this `PUT` operation will produce the static CDS external entity `ZMY_CDS_STATIC_EXT_ENTITY` with the following source:

> ### Sample Code:  
> ```abap
> @EndUserText.label: 'My external entity'
> define external entity ZMY_CDS_STATIC_EXT_ENTITY external name "My remote object name"
> {
>   key key_field : abap.int1;
>   field : abap.char(10) external name "My field external name";
> }
> WITH FEDERATED DATA
> PROVIDED BY ZMY_LOGICAL_EXT_SCHEMA
> ```
