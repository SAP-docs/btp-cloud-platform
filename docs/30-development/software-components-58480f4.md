...
A software component is the container that organizes and transports your ABAP development. It bundles all development objects of at least one runnable application under a top-level structure package, with sub-packages of type *Development* holding the actual objects.



## Concept

Your development is organized and managed in software components. In most cases, a software component is designed to contain all development objects of at least one application and should be runnable itself. However, you can distribute your development across multiple software components. This approach works well when development parts have independent lifecycles or when you need to separate basic reuse functionality from application-specific functionality.

The software component `ZLOCAL` is available by default. It works like `$TMP` in a classic ABAP system: use it for non-transportable, local development. You can't create development objects directly in the structure package. First, create a sub-package of type *Development*. Then, start developing in it. The project structure looks as follows:

> ### Example:  
> `Z_BONUS_MANAGEMENT` \(top-level package, type Structure - created automatically with the software component of the same name\)
> 
...
> 
> `/ACME/BONUS_MANAGEMENT` \(top-level package, type Structure - created automatically with the software component of the same name\)
> 
> `/ACME/BONUS_CALCULATION` \(sub-package, type Development\)
> 
> `/ACME_BONUS_CONFIG` \(sub-package, type Development\)
...
## Using Objects Across Software Components
